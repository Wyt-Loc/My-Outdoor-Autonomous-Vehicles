...
# 绘图
class myThread_huizhi(threading.Thread):
    ...
    def run(self):
        ...
        # 创建绘制实时损失的动态窗口
        plt.ion()
        while self.stop and OK:
            if self.show and xyz:
                # 显示的数据
                plt.clf()  # 清除之前画的图
                fig = plt.gcf()  # 获取当前图
                ...
                ax.view_init(self.elev, self.azim)  # 设定角度
                # 距离远近颜色递变 c = np.sqrt(np.array(Z[1:]) * np.array(Z[1:]) + (np.array(X1[1:]) * np.array(X1[1:])) + np.array(
                #        X2[1:]) * np.array(X2[1:]))
                # c = np.sqrt((np.array(x[1:]) * np.array(x[1:])) + (np.array(y[1:]) * np.array(y[1:])))  # 横向距离递变
                # c = np.array(z)
                ax.scatter([xcar], [ycar], [zcar], c='r', s=50, cmap='rainbow', marker='^')  # 原点绘制提示
                ax.scatter(x, y, z, s=2, cmap='rainbow', marker='.')
                # self.elev, self.azim = ax.elev, ax.azim  # 将当前绘制的三维图角度记录下来，用于下一次绘制（放在ioff()函数前后都可以，但放在其他地方就不行）
                # self.elev, self.azim = ax.elev, ax.azim - 1  # 可自动旋转角度，不需要人去拖动
...
class myThread_zitai(threading.Thread):
        ...
        if self.jiuzhou_data:  # 非空
            if len(self.jiuzhou_data) == len(self.lasetime)-1:  # 且等大，否则就是有问题
                for i in range(0, len(self.jiuzhou_data)):
                    ...
                    if self.jiuzhou_data[i][3]-self.lastxj < 300 and self.jiuzhou_data[i][4]-self.lastyj < 300 and self.jiuzhou_data[i][5]-self.lastzj < 300:  # 防止突变，其实还是数据验证有问题
    ...
    def run(self):
        self.print("线程启动")
        try:
            self.ser = serial.Serial("COM8", 921600)  # 位置1
        ...
        for id, i in enumerate(data):
            if i & 0x80 == 0x00 \
                ...
                if self.jiaoyan(data[id], data[id+1], data[id+2], data[id+3]):
                    """按照文档解析数据,单位是cm和角度"""
                    juli = ((data[id] & 0x0f) << 7) | ((data[id + 1] & 0x7f) << 1) | ((data[id+2] & 0x40) >> 6)  # 按文档的说法第一项位移8位才对，我也这么认为，但是案例中代码表示位移7就对了，这样的话，实际上是A0和B6是存在或的关系的，而不是简单的排列了，但是事实就是如此，所以说这里绝对存在问题，是厂家的下位机的问题，而上位机人员进行了妥协，但是文档缺还是按照设计之初所想的那样写的，这是我的理解
...
class myThread_uwb(threading.Thread):
    ...
    def jiexi(self):
        ...
        while len(self.data) > 0:
            try:
                if self.data[0] == 0x01 and self.data[0+1] == 0x03 and self.data[0+2] == 0x2a and self.data[0+3] == 0x00 and self.data[0+4] == 0x00:
                    # print(self.data)
