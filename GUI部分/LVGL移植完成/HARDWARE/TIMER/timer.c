...
/***************************************************************
【函数名】 void Drive_io_init(void)
【功  能】 初始化电机驱动引脚  控制正反转 IN1 -- B5   IN2 --- B6
【参数值】 无
【返回值】 无
****************************************************************/
void Drive_io_init(void){
 ...
 RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE); //使能外设时钟总线

 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2 | GPIO_Pin_3; 							 //初始化外设引脚
 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 	   // 输出模式为 推挽输出
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 // 速率 50MHZ
 GPIO_Init(GPIOB, &GPIO_InitStructure);								 // 初始化结构体
 GPIO_ResetBits(GPIOB, GPIO_Pin_2 | GPIO_Pin_3);					 						 // 初始化拉低
...
{  
   ...
   //设置该引脚为复用输出功能,输出TIM4 CH1 2的PWM脉冲波形	GPIOB.6 7
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7; //TIM_CH2
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //复用推挽输出  这块一定要是这个 PWM属于特殊输出
