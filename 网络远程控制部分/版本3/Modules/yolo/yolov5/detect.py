...
import 网络远程控制部分.版本3.Modules.自动避障.EmergencyStop
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                ...
                # print("画面显示")
                # 这块是自己添加的了
                # cv2.circle(im0, (300, 100), 3, (255, 0, 0), 3)
                # print("name = ", str(names[c]))
                cv2.imshow("left", im0)
...
class Control_start(网络远程控制部分.版本3.Modules.Control.MyControl.Mykey):
    pass
...
def start():
    yl = YoloAndLidar()
    t1 = threading.Thread(target=yl.run111)
    t1.start()
    t2 = threading.Thread(target=yolo_start)
    t2.start()
    t3 = threading.Thread(target=网络远程控制部分.版本3.Modules.Control.MyControl.Control_start)
    t3.start()
