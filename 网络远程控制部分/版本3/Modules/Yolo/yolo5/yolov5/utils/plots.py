...
# Settings
from yolov5 import camera_configs
...
def save_one_box(xyxy, im, file=Path('im.jpg'), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    # 打印坐标信息
    print("左上(" + str(int(xyxy[0, 0])) + "," + str(int(xyxy[0, 1])) + "),右下(" + str(int(xyxy[0, 2])) + "," + str(
        int(xyxy[0, 3])) + ")")

    x = int(xyxy[0, 2] - xyxy[0, 0])  # 竖着
    y = int(xyxy[0, 3] - xyxy[0, 1])  # 横向
    print("x = ", x)
    print("y = ", y)
    ...
    # 如果有信息，那么开始测距，测量出现的所有物体在此时的距离
    # 给出物体的坐标我们采取测量坐标中间值的方法，
    # 为了防止测得数据错误，我们将这一区域填充为最多的深度值，但是也不能保证每次都能测到结果
    # 另一种思路就是，我们以Yolo相机为主体，yolo能检测到，那么我们可以认为这个地方大概率是有东西存在的，
    # 那么我们测量这个范围内深度值，如果存在深度值不是最大且大于整体一定比例的情况，那么我们选择这个深度值作为
    # 当前深度，进行测量或者这两个条件进行一个 or 的关系 来尽可能的保证距离是可以测得的

    frame1 = im[0:480, 0:640]
    frame2 = im[0:480, 640:1280]  # 割开双目图像
